...
if __name__ == "__main__":
    ...
    flow = RealNVPTabular(num_coupling_layers=10, in_dim=3,
                          num_layers=3, hidden_dim=512).cuda()
    ...
    negative_softmax[:, 0] = (torch.ones(
        negative_labels.shape) - negative_labels).reshape(-1)

    negative_data = torch.hstack(
        (negative_instance_features, negative_softmax))
    ...
    positive_softmax[:, 0] = (torch.ones(
        positive_labels.shape) - positive_labels).reshape(-1)

    positive_data = torch.hstack(
        (positive_instance_features, positive_softmax))
    ...
    for t in tqdm(range(EPOCHS)):
        for local_batch, local_labels in (positive_loader):
            ...
            local_prediction = model_prediction(predictive_model, features_cuda).detach().cpu()


            sldj = flow.logdet()
            flow_loss = loss_fn(z, sldj)
            ce_loss = loss_cefn.forward(local_labels, positive=True)
            total_loss = ce_loss
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
    ...
    for t in tqdm(range(EPOCHS)):
        for local_batch, local_labels in (negative_loader):
            ...
            sldj = flow.logdet()
            flow_loss = loss_fn(z, sldj)
            ce_loss = loss_cefn.forward(local_labels)
            total_loss = ce_loss
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
    ...
    for local_batch, local_labels in (negative_loader):
        ...
        labels = model_prediction(
            predictive_model, x_value).detach().cpu()
    ...
    # input_value = local_batch
    # z_value = get_latent_representation_from_flow(flow_model, input_value)
    # x_value = original_space_value_from_latent_representation(flow_model, z_value)
    # prediction =
